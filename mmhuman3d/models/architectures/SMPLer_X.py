...
class SMPLer_X(BaseArchitecture, metaclass=ABCMeta):
    ...
    def _prepare_body_module(self):
        ...
        self.body_regressor = self.body_rotation_net
    ...
    def forward_test(self,  img: torch.Tensor, img_metas: dict, **kwargs):
        ...
        for bbox in (lhand_bbox, rhand_bbox, face_bbox):
            bbox[:, 0] *= self.input_img_shape[1] / self.input_body_shape[1]
            bbox[:, 1] *= self.input_img_shape[0] / self.input_body_shape[0]
            bbox[:, 2] *= self.input_img_shape[1] / self.input_body_shape[1]
            bbox[:, 3] *= self.input_img_shape[0] / self.input_body_shape[0]
